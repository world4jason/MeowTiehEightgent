# `@meowtieheightgent/plugin-sdk`

Official TypeScript SDK for Mth plugin authors.

- **Worker SDK:** `@meowtieheightgent/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@meowtieheightgent/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@meowtieheightgent/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@meowtieheightgent/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@meowtieheightgent/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@meowtieheightgent/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@meowtieheightgent/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@meowtieheightgent/plugin-sdk/ui/hooks` | Hooks only |
| `@meowtieheightgent/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@meowtieheightgent/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@meowtieheightgent/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@meowtieheightgent/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@meowtieheightgent/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@meowtieheightgent/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @meowtieheightgent/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main Mth app. They can call ordinary Mth HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Mth instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@meowtieheightgent/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@meowtieheightgent/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Mth information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@meowtieheightgent/plugin-sdk`.
...
import { usePluginAction, type PluginBridgeError } from "@meowtieheightgent/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@meowtieheightgent/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@meowtieheightgent/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@meowtieheightgent/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@meowtieheightgent/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@meowtieheightgent/plugin-sdk/ui";
...
import {
  useHostContext,
  usePluginAction,
} from "@meowtieheightgent/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@meowtieheightgent/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@meowtieheightgent/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@meowtieheightgent/plugin-sdk/bundlers";
...
```bash
mth-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@meowtieheightgent/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__mth__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__mth__/events` streams `reload` SSE events on UI build changes
