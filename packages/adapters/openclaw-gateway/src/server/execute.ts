...
import { asNumber, asString, buildMthEnv, parseObject } from "@meowtieheightgent/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "mth";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "mth";
  if (input.strategy === "run") return `mth:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `mth:issue:${input.issueId}`;
  return fallback;
}
...
function resolveMthApiUrlOverride(value: unknown): string | null {
...
function buildMthEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const mthApiUrlOverride = resolveMthApiUrlOverride(ctx.config.mthApiUrl);
  const mthEnv: Record<string, string> = {
    ...buildMthEnv(ctx.agent),
    MTH_RUN_ID: ctx.runId,
  };

  if (mthApiUrlOverride) {
    mthEnv.MTH_API_URL = mthApiUrlOverride;
  }
  if (wakePayload.taskId) mthEnv.MTH_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) mthEnv.MTH_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) mthEnv.MTH_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) mthEnv.MTH_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) mthEnv.MTH_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    mthEnv.MTH_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return mthEnv;
}

function buildWakeText(payload: WakePayload, mthEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/mth-claimed-api-key.json";
  const orderedKeys = [
    "MTH_RUN_ID",
    "MTH_AGENT_ID",
    "MTH_COMPANY_ID",
    ...
    "MTH_APPROVAL_ID",
    "MTH_APPROVAL_STATUS",
    "MTH_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = mthEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = mthEnv.MTH_API_URL ?? "<set MTH_API_URL>";

  const lines = [
    "Mth wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `MTH_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load MTH_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $MTH_API_KEY on every API call.",
    "- Use X-Mth-Run-Id: $MTH_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: MTH_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$MTH_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$MTH_COMPANY_ID/issues?assigneeAgentId=$MTH_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardMthPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  mthEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateMth = parseObject(payloadTemplate.mth);
  const workspace = asRecord(ctx.context.mthWorkspace);
  const workspaces = Array.isArray(ctx.context.mthWorkspaces)
    ? ctx.context.mthWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.mthRuntimeServiceIntents)
    ? ctx.context.mthRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardMth: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: mthEnv.MTH_API_URL ?? null,
  };

  if (workspace) {
    standardMth.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardMth.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardMth.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "mth-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const mthEnv = buildMthEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, mthEnv);
  ...
  const mthPayload = buildStandardMthPayload(ctx, wakePayload, mthEnv, payloadTemplate);
