...
import { asString, parseObject } from "@meowtieheightgent/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Mth server host.",
        });
      }
    } catch (err) {
      checks.push({
