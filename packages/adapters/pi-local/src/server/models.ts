...
import type { AdapterModel } from "@meowtieheightgent/adapter-utils";
import { asString, runChildProcess } from "@meowtieheightgent/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.MTH_PI_COMMAND === "string" &&
    process.env.MTH_PI_COMMAND.trim().length > 0
      ? process.env.MTH_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["MTH_", "npm_", "NPM_"] as const;
