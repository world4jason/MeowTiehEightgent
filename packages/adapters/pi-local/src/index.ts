...
Use when:
- You want Mth to run Pi (the AI coding agent) locally as the agent runtime
...
- Mth requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/mth-sessions/ and resumed with --session.
