...
function renderMthEnvNote(env: Record<string, string>): string {
  const mthKeys = Object.keys(env)
    .filter((key) => key.startsWith("MTH_"))
    .sort();
  if (mthKeys.length === 0) return "";
  return [
    "Mth runtime note:",
    `The following MTH_* environment variables are available in this run: ${mthKeys.join(", ")}`,
...
export async function ensureCursorSkillsInjected(
  ...
  const skillsEntries = options.skillsEntries
    ?? (options.skillsDir
      ...
      : await readMthRuntimeSkillEntries({}, __moduleDir));
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[mth] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[mth] Removed maintainer-only Cursor skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of skillsEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[mth] ${result === "repaired" ? "Repaired" : "Injected"} Cursor skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[mth] Failed to inject Cursor skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const cursorSkillEntries = await readMthRuntimeSkillEntries(config, __moduleDir);
  const desiredCursorSkillNames = resolveMthDesiredSkillNames(config, cursorSkillEntries);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[mth] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const mthEnvNote = renderMthEnvNote(env);
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: mthEnvNote.length,
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[mth] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
