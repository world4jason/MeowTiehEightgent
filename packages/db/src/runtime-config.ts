...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "mth-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveDefaultConfigPath(): string {
  return path.resolve(
    resolveMthHomeDir(),
    "instances",
    resolveMthInstanceId(),
    CONFIG_BASENAME,
  );
}

function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveMthHomeDir(), "instances", resolveMthInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".meowtieheightgent", CONFIG_BASENAME);
...
function resolveMthConfigPath(): string {
  if (process.env.MTH_CONFIG?.trim()) {
    return path.resolve(process.env.MTH_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveMthEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveMthEnvPath(configPath);
