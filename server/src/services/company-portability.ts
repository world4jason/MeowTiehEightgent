...
import {
  readMthSkillSyncPreference,
  writeMthSkillSyncPreference,
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".mth.yaml" || normalized === ".mth.yml") return "extension";
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[mthExtensionPath];
  if (selectedFiles.has(mthExtensionPath) && typeof extensionEntry === "string") {
    filtered[mthExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findMthExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".mth.yaml"] === "string") return ".mth.yaml";
  if (typeof files[".mth.yml"] === "string") return ".mth.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.mth.yaml") || entry.endsWith("/.mth.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "mth_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "meowtieheightgent/mth",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/meowtieheightgent/mth/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "mth_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.mthSkillKey = skill.key;
  metadata.mth = {
    ...(isPlainRecord(metadata.mth) ? metadata.mth : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const mthExtensionPath = findMthExtensionPath(normalizedFiles);
  const mthExtension = mthExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, mthExtensionPath) ?? "")
    : {};
  const mthCompany = isPlainRecord(mthExtension.company) ? mthExtension.company : {};
  const mthAgents = isPlainRecord(mthExtension.agents) ? mthExtension.agents : {};
  const mthProjects = isPlainRecord(mthExtension.projects) ? mthExtension.projects : {};
  const mthTasks = isPlainRecord(mthExtension.tasks) ? mthExtension.tasks : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(mthCompany.brandColor),
      logoPath: asString(mthCompany.logoPath) ?? asString(mthCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof mthCompany.requireBoardApprovalForNewAgents === "boolean"
          ? mthCompany.requireBoardApprovalForNewAgents
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(mthAgents[slug]) ? mthAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(mthProjects[slug]) ? mthProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(mthTasks[slug]) ? mthTasks[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function exportBundle(
    ...
    const mthAgentsOut: Record<string, Record<string, unknown>> = {};
    const mthProjectsOut: Record<string, Record<string, unknown>> = {};
    const mthTasksOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readMthSkillSyncPreference(
        ...
        mthAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      mthProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      mthTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const mthExtensionPath = ".mth.yaml";
    const mthAgents = Object.fromEntries(
      Object.entries(mthAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const mthProjects = Object.fromEntries(
      Object.entries(mthProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const mthTasks = Object.fromEntries(
      Object.entries(mthTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[mthExtensionPath] = buildYamlFile(
      {
        schema: "mth/v1",
        ...
        agents: Object.keys(mthAgents).length > 0 ? mthAgents : undefined,
        projects: Object.keys(mthProjects).length > 0 ? mthProjects : undefined,
        tasks: Object.keys(mthTasks).length > 0 ? mthTasks : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, mthExtensionPath);
  ...
  async function buildPreview(
    ...
    if (include.issues) {
      for (const issue of manifest.issues) {
        ...
        if (issue.recurrence) {
          warnings.push(`Task ${issue.slug} has recurrence metadata; Mth will import it as a one-time issue for now.`);
        }
      }
    }
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeMthSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
