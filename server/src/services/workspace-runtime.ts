...
import type { AdapterRuntimeServiceReport } from "@meowtieheightgent/adapter-utils";
import type { Db } from "@meowtieheightgent/db";
import { workspaceRuntimeServices } from "@meowtieheightgent/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.MTH_WORKSPACE_CWD = input.worktreePath;
  env.MTH_WORKSPACE_PATH = input.worktreePath;
  env.MTH_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.MTH_WORKSPACE_BRANCH = input.branchName;
  env.MTH_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.MTH_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.MTH_WORKSPACE_SOURCE = input.base.source;
  env.MTH_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.MTH_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.MTH_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.MTH_PROJECT_ID = input.base.projectId ?? "";
  env.MTH_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.MTH_AGENT_ID = input.agent.id;
  env.MTH_AGENT_NAME = input.agent.name;
  env.MTH_COMPANY_ID = input.agent.companyId;
  env.MTH_ISSUE_ID = input.issue?.id ?? "";
  env.MTH_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.MTH_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.MTH_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.MTH_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.MTH_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.MTH_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.MTH_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.MTH_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.MTH_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.MTH_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.MTH_PROJECT_ID = input.workspace.projectId ?? "";
  env.MTH_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.MTH_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".meowtieheightgent", "worktrees");
