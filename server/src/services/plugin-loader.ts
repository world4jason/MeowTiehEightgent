 ...
 *    (`~/.meowtieheightgent/plugins/`) and `node_modules` for packages matching
 *    the `mth-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Mth plugins.
 * Packages matching this pattern are considered Mth plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "mth-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: MthPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.meowtieheightgent/plugins/ local directory
  | "npm"               // npm packages matching mth-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: MthPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
   ...
   * Defaults to ~/.meowtieheightgent/plugins/
   */
  ...
  /**
   * Whether to discover installed npm packages matching the mth-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "mth-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: MthPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Mth plugins installed as npm packages in the current
   * Node.js environment matching the "mth-plugin-*" naming convention.
   *
   ...
   * the "mthPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Mth plugin.
   * Throws if the package is a Mth plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<MthPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: MthPluginManifestV1;
    newManifest: MthPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Mth plugin naming convention.
 * Accepts both the "mth-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @meowtieheightgent/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "mthPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const mthPlugin = pkgJson["mthPlugin"];
  if (
    mthPlugin !== null &&
    typeof mthPlugin === "object" &&
    !Array.isArray(mthPlugin)
  ) {
    const manifestRelPath = (mthPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: MthPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumMthVersion;
}
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: MthPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as MthPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Mth plugin (no manifest found).`,
      );
    }
   ...
   * if the package is not a Mth plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasMthPlugin = "mthPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasMthPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<MthPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: MthPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @meowtieheightgent/shared exports). Run those workers through
