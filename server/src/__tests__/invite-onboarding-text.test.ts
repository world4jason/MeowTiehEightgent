...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Mth OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Mth base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("mthApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.mthApiUrl");
    expect(text).toContain("~/.openclaw/workspace/mth-claimed-api-key.json");
    expect(text).toContain("MTH_API_KEY");
