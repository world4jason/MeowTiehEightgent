...
import type { DeploymentExposure, DeploymentMode } from "@meowtieheightgent/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "mth" &&
    normalized !== "mth-create-agent" &&
    normalized !== "mth-create-plugin" &&
...
/** Resolve the Mth repo skills directory (built-in / managed skills). */
function resolveMthSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const mthSkillsDir = resolveMthSkillsDir();

  // Build set of Mth-managed skill names
  const mthSkillNames = new Set<string>();
  if (mthSkillsDir) {
    try {
      for (const entry of fs.readdirSync(mthSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) mthSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isMthManaged: mthSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.mthApiUrl)) {
    const legacyMthApiUrl = nonEmptyTrimmedString(input.mthApiUrl);
    if (legacyMthApiUrl) merged.mthApiUrl = legacyMthApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    mthApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.mthApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawMthApiUrl =
    typeof defaults.mthApiUrl === "string"
      ? defaults.mthApiUrl.trim()
      : "";
  if (rawMthApiUrl) {
    try {
      const parsedMthApiUrl = new URL(rawMthApiUrl);
      if (
        parsedMthApiUrl.protocol !== "http:" &&
        parsedMthApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_mth_api_url_protocol",
          level: "warn",
          message: `mthApiUrl must use http:// or https:// (got ${parsedMthApiUrl.protocol}).`
        });
      } else {
        normalized.mthApiUrl = parsedMthApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_mth_api_url_configured",
          level: "info",
          message: `mthApiUrl set to ${parsedMthApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_mth_api_url_invalid",
        level: "warn",
        message: `Invalid mthApiUrl: ${rawMthApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Mth host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Mth is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm meowtieheightgent allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/mth";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/mth-claimed-api-key.json and load MTH_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: mthApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Mth hostname is reachable and allowed via `pnpm meowtieheightgent allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Mth API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/mth/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Mth OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Mth generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "mthApiUrl": "https://mth-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Mth before key claim is allowed.
    ...
    - ~/.openclaw/workspace/mth-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/mth-claimed-api-key.json

    And set the MTH_API_KEY and MTH_API_URL in your environment variables as specified here:
    ...
    Then set MTH_API_KEY and MTH_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Mth skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your MTH_API_URL to the top of your skill and note the path to your MTH_API_URL
    ...
    ${
      ...
      "Ensure Mth is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Mth base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.mthApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm meowtieheightgent allowed-hostname <host>
      - then restart Mth and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@mth.local";
    ...
    res.json({
      skills: [
        ...
        },
        {
          ...
          path: "/api/skills/mth-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              mthApiUrl: req.body.mthApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.mthApiUrl &&
          !persistedDefaults.mthApiUrl
        ) {
          missingPersistedFields.push("mthApiUrl");
        }
