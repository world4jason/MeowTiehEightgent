...
import { agents as agentsTable, companies, heartbeatRuns } from "@meowtieheightgent/db";
...
import { runClaudeLogin } from "@meowtieheightgent/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@meowtieheightgent/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@meowtieheightgent/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@meowtieheightgent/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@meowtieheightgent/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeMthSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readMthSkillSyncPreference(
