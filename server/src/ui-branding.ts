const FAVICON_BLOCK_START = "<!-- MTH_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- MTH_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- MTH_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- MTH_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.MTH_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.MTH_WORKTREE_NAME) ?? nonEmpty(env.MTH_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.MTH_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="mth-worktree-enabled" content="true" />',
    `<meta name="mth-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="mth-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="mth-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
