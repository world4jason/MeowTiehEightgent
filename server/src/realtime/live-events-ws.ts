...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@meowtieheightgent/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  mthUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).mthUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.mthUpgradeContext = context;
