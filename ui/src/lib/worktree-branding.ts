...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("mth-worktree-enabled") !== "true") return null;

  const name = readMetaContent("mth-worktree-name");
  const color = normalizeHexColor(readMetaContent("mth-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("mth-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
