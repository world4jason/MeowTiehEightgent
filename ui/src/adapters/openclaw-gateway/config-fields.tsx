...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Mth API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "mthApiUrl",
                  String(config.mthApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "mthApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://mth.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "mth"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Mth persists a device key during onboarding so pairing approvals
