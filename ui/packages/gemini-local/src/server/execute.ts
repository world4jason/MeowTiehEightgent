...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "MTH_API_URL") || !hasNonEmptyEnvValue(env, "MTH_API_KEY")) return "";
  return [
    "Mth API access note:",
    "Use run_shell_command with curl to make Mth API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $MTH_API_KEY\\" \\"$MTH_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $MTH_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-Mth-Run-Id: $MTH_RUN_ID\\" -d '{...}' \\"$MTH_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
/**
 * Inject Mth skills directly into `~/.gemini/skills/` via symlinks.
...
async function ensureGeminiSkillsInjected(
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[mth] Failed to prepare Gemini skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[mth] Removed maintainer-only Gemini skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of skillsEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[mth] ${result === "repaired" ? "Repaired" : "Linked"} Gemini skill: ${entry.name}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[mth] Failed to link Gemini skill "${entry.name}": ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[mth] Gemini session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: mthEnvNote.length + apiAccessNote.length,
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[mth] Gemini resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
