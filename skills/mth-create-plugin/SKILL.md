---
name: mth-create-plugin
description: >
  Create new Mth plugins with the current alpha SDK/runtime. Use when
...
Use this skill when the task is to create, scaffold, or document a Mth plugin.
...
```bash
pnpm --filter @meowtieheightgent/create-mth-plugin build
node packages/plugins/create-mth-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Mth repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.mth-sdk/`:

```bash
pnpm --filter @meowtieheightgent/create-mth-plugin build
node packages/plugins/create-mth-plugin/dist/index.js @acme/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/meowtieheightgent/packages/plugins/sdk
```
...
- is installed into Mth from an absolute local path during development
