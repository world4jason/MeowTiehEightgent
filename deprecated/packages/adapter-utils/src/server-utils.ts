...
const MTH_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface MthSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: MthSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "mth_required",
      originLabel: "Required by Mth",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Mth",
    readOnly: false,
  };
}
...
export function buildMthEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    MTH_AGENT_ID: agent.id,
    MTH_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.MTH_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.MTH_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.MTH_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.MTH_API_URL = apiUrl;
  return vars;
}
...
export async function resolveMthSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...MTH_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listMthSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<MthSkillEntry[]> {
  const root = await resolveMthSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `meowtieheightgent/mth/${entry.name}`,
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Mth skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Mth cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredMthRuntimeSkills(value: unknown): MthSkillEntry[] {
...
export async function readMthRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<MthSkillEntry[]> {
  const configuredEntries = normalizeConfiguredMthRuntimeSkills(config.mthRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listMthSkillEntries(moduleDir, additionalCandidates);
}

export async function readMthSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listMthSkillEntries(moduleDir);
...
export function readMthSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.mthSkillSync;
...
function canonicalizeDesiredMthSkillReference(
  reference: string,
...
export function resolveMthDesiredSkillNames(
  config: Record<string, unknown>,
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredMthSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeMthSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.mthSkillSync;
  const current =
  ...
  next.mthSkillSync = current;
  return next;
}

export async function ensureMthSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Mth server itself is started from
    // within a Claude Code session (e.g. `npx meowtieheightgent run` in a terminal
