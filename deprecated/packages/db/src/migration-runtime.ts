...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://mth:mth@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "mth");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://mth:mth@127.0.0.1:${preferredPort}/mth`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://mth:mth@127.0.0.1:${port}/mth`,
  ...
  const adminConnectionString = `postgres://mth:mth@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "mth");

  return {
    connectionString: `postgres://mth:mth@127.0.0.1:${selectedPort}/mth`,
