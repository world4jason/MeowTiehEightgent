...
function resolveMthInstanceId(): string {
  const raw = process.env.MTH_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveMthHomeDir(), "instances", resolveMthInstanceId(), "config.json");
}
...
function resolveConnectionString(config: PartialConfig | null): string {
  ...
  return `postgres://mth:mth@127.0.0.1:${port}/mth`;
}

function resolveDefaultBackupDir(): string {
  return path.resolve(resolveMthHomeDir(), "instances", resolveMthInstanceId(), "data", "backups");
}
