...
export const pluginManifestV1Schema = z.object({
    ...
    minimumMthVersion: z.string().regex(/^\d+\.\d+\.\d+(-[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?(\+[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?$/, "minimumMthVersion must follow semver (e.g. 1.0.0)").optional(),
    ...
    if (manifest.minimumHostVersion
        && manifest.minimumMthVersion
        && manifest.minimumHostVersion !== manifest.minimumMthVersion) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "minimumHostVersion and minimumMthVersion must match when both are declared",
