...
/**
 * Filter .mth.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterMthYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://mth.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm meowtieheightgent company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Mth](https://mth.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Mth](https://mth.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .mth.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .mth.yaml
    const yamlPath = exportData.mthExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterMthYaml(exportData.files[yamlPath], checkedFiles);
    }
