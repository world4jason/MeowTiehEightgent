...
type MthSprite = (typeof MTH_SPRITES)[number];
...
function spriteSize(sprite: MthSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = MTH_SPRITES[(baseCol + baseRow) % MTH_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = MTH_SPRITES[Math.floor(Math.random() * MTH_SPRITES.length)]!;
