...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@meowtieheightgent/db";
...
import type {
  ...
  PluginWorkspace,
  IssueComment,
} from "@meowtieheightgent/plugin-sdk";
 ...
 *
 * This implementation delegates to the core Mth domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@meowtieheightgent/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
